...
function convertShorthandToDate(shorthand) {
  ...
  return new Date('20' + year, month - 1, day).toDateString();
}

function App() {
  const dateOptions = Object.keys(dates).reverse();
  const [nextValue, setNextValue] = useState(dateOptions[0]);
  const [prevValue, setPrevValue] = useState(dateOptions[1]);

  const formattedOptions = dateOptions.map(dateOption => ({
    value: dateOption,
    ...
    <div className="App">
      ...
      <div className="date-picker-row">
        ...
        <Dropdown
          options={formattedOptions}
          onChange={newValue => {
            setNextValue(newValue.value);
          }}
          value={nextValue}
          placeholder="Select a date to compare to"
        />
      </div>
      {dates[nextValue] === dates[prevValue] ? (
        <NoUpdates prevDate={prevValue} nextDate={nextValue} />
      ) : (
        <ReactDiffViewer
          oldValue={dates[prevValue]}
          newValue={dates[nextValue]}
          splitView={true}
